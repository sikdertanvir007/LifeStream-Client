...
const AllDonationRequests = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axiosSecure.get(`/admin-donation-requests?${params.toString()}`);
      return res.data;
    },
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(["all-donation-requests"]),
  });
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl text-red-500 font-bold">All Donation Requests</h2>
      </div>
      ...
      <div className="flex justify-center items-center space-x-2 mt-4">
        ...
        {[...Array(totalPages).keys()].map((n) => (
          <button
            key={n + 1}
            className={`btn btn-outline btn-sm ${page === n + 1 ? "btn-active" : ""}`}
            onClick={() => setPage(n + 1)}
          >
            {n + 1}
          </button>
